...
// @include        http*://*desi.cd/upload.php*
// @include        http*://*desi.cd/requests.php*
// @include        http*://*desi.cd/torrents.php*
...
var factory = {
    ...
    locations : new Array(
        {
            name : 'desicd_upload',
            regex : /http(s)?\:\/\/(.*\.)?desi\.cd\/upload\.php.*/i
        },
        {
            name : 'desicd_edit',
            regex : /http(s)?\:\/\/(.*\.)?desi\.cd\/torrents\.php\?action=editgroup&groupid=.*/i
        },
        {
            name : 'desicd_request',
            regex : /http(s)?\:\/\/(.*\.)?desi\.cd\/requests\.php\?action=new/i
        },
        {
            name : 'desicd_request_edit',
            regex : /http(s)?\:\/\/(.*\.)?desi\.cd\/requests\.php\?action=edit&id=.*/i
        },
        {
            name : 'desicd_torrent_overview',
            regex : /http(s)?\:\/\/(.*\.)?desi\.cd\/torrents\.php\?id=.*/i
        },
        {
            name : 'waffles_upload',
...
//            name : 'waffles_upload_new',
    ...
    setDefaultFormat : function() {
        ...
        if (default_format !== null && default_format in format_offsets) {
            ...
            switch (this.currentLocation) {
                ...
                default:
                    format_select.selectedIndex = format_offsets[defaultDesiFormat];
                    break;
            }
        }
    },
    ...
    setSelect : function(select, data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            // incompatibility with jQuery in Chrome which will make it impossible to add a new artist field on desi.cd
